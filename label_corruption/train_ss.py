...
parser.add_argument('--val_set_size', type=int, default=0, help='if 0, then a val set is not used')
...
parser.add_argument('--no_ce', action='store_true', help='Turns off standard CE loss; for pre-training with SS loss only')
...
if args.dataset == 'cifar10':
    train_data = CIFAR10(args.data_path, True, False, 0,
        corruption_prob=args.corruption_prob, corruption_type='unif', transform=train_transform)
    if args.val_set_size > 0:
	    train_data_clean = CIFAR10(args.data_path, True, False, 0,
	        corruption_prob=0, corruption_type='unif', transform=test_transform)
	    val_indices = np.random.choice(len(train_data), args.val_set_size, replace=False)
	    train_indices = np.array(list(set(range(len(train_data))) - set(val_indices)))
    ...
    train_data = CIFAR100(args.data_path, True, False, 0,
        corruption_prob=args.corruption_prob, corruption_type='unif', transform=train_transform)
    if args.val_set_size > 0:
	    train_data_clean = CIFAR100(args.data_path, True, False, 0,
	        corruption_prob=0, corruption_type='unif', transform=test_transform)
	    val_indices = np.random.choice(len(train_data), args.val_set_size, replace=False)
	    train_indices = np.array(list(set(range(len(train_data))) - set(val_indices)))
	    val_data = torch.utils.data.Subset(train_data_clean, val_indices)
...
def test():
    ...
    loss_avg = 0.0
    correct = 0
    eval_loader = val_loader if (args.val_set_size > 0) else test_loader
    with torch.no_grad():
